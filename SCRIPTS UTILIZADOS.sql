...
CREATE TABLE tareas (
    ...
    Carnet VARCHAR(20) NOT NULL, -- Carnet del estudiante (llave for�nea)
    nota1 INT NOT NULL, -- Nota 1
    nota2 INT NOT NULL, -- Nota 2
    nota3 INT NOT NULL, -- Nota 3
    nota4 INT NOT NULL, -- Nota 4
    CONSTRAINT FK_tareas_Tb_alumnos FOREIGN KEY (Carnet) REFERENCES Tb_alumnos(carnet) -- Relaci�n con Tb_alumnos
);
...
INSERT INTO tareas (Carnet, nota1, nota2, nota3, nota4) VALUES
('0905-10-1279', 81, 67, 82, 81),
-- ... (todos los dem�s registros proporcionados)
('0905-24-24315', 47, 69, 33, 25);
...
CONSTRAINT FK_asistencia_alumnos FOREIGN KEY (Carnet)
REFERENCES Tb_alumnos(Carnet)
ON DELETE CASCADE
ON UPDATE CASCADE,

CONSTRAINT CK_asistencia_presente CHECK (presente IN ('S', 'N')),

CONSTRAINT UQ_asistencia_carnet_fecha UNIQUE (Carnet, fecha)
);

INSERT INTO asistencia (Carnet, fecha, presente) VALUES
('0905-10-1279', '2023-03-01', 'S'),
('0905-15-9622', '2023-03-01', 'N' ),
('0905-17-20273', '2023-03-02', 'S'),
('0905-21-4966', '2023-03-02', 'S' ),
('0905-24-24315', '2023-03-03', 'N');
...
CONSTRAINT FK_notas_finales_alumnos FOREIGN KEY (Carnet)
REFERENCES Tb_alumnos(Carnet)
ON DELETE CASCADE
ON UPDATE CASCADE,

CONSTRAINT CK_nota_final_rango CHECK (nota_final >= 0 AND nota_final <=100),
);
INSERT INTO notas_finales (Carnet, nota_final) 
VALUES
('0905-10-1279', 81.5),
('0905-15-9622', 84.25),
('0905-17-20273', 88.75),
...
SET seccion = 
    CASE 
        WHEN seccion = 'A' THEN 'B'
        WHEN seccion = 'B' THEN 'A'
        ELSE seccion
    END
	Where seccion IN ('A', 'B');
...
SELECT a.carnet, a. Estudiante, t.nota1, t.nota2, t.nota3, t.nota4
FROM Tb_alumnos a
JOIN tareas t ON a.carnet = t.carnet;

UPDATE notas_finales
SET nota_final = 90.00
WHERE carnet = '0905-10-1279';

WITH AsistenciaAEliminar AS (
    SELECT TOP 1 id_asistencia
    FROM asistencia
    WHERE Carnet = '0905-15-14297'
    ORDER BY fecha ASC
)
DELETE FROM asistencia
WHERE id_asistencia IN (SELECT id_asistencia FROM AsistenciaAEliminar);


SELECT
    a.Estudiante,
    MAX(asist.fecha) AS ultima_asistencia,
    CASE
        WHEN MAX(CASE WHEN asist.presente = 'S' THEN 1 ELSE 0 END) = 1
        THEN 'Presente'
        ELSE 'Ausente'
    END AS estado,
    nf.[nota_final]
FROM
    Tb_alumnos a
LEFT JOIN
    asistencia asist ON a.carnet = asist.Carnet
LEFT JOIN
    notas_finales nf ON a.carnet = nf.Carnet
GROUP BY
    a.carnet,
    a.Estudiante,
    nf.[nota_final]
